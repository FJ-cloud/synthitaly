...
| `src/synthitaly/` (Italy, active) | 4,795 LOC, 8 files, 143 tests | current |

An honest observation worth including: the active version was written *because* 3,440 lines
were unreadable, and has since grown to 4,795 — but across **eight files instead of
twenty-five**, a large part of it analysis tooling (`features.py`, `diagnostics.py`,
`panel.py`, `creditscoring.py`) added after the model itself was finished. The size reduction
...
- The **ODD protocol** (Grimm et al. 2020) — `docs/ODD.md`, written 2026-06-30 `f56b8b7`,
...
|---|---|---|
| **Automated / unit tests** | yes | **143 tests across 11 files** (`pytest --collect-only`, verified). Growth is itself documented: 29 tests (18 May) → 39 → 48 (25 May) → 96 → 143 |
